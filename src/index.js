...
function createDefaultProject() {
  ...
  const item1 = taskItem(
    "This is a task",
    `Great you learned how to view task descriptions.`,
    "01/01/2020",
    "Low",
    1,
    false,
    ++project1.taskCounter
  );
...
if (projectList.projectListObject == null) {
  ...
  if (
    Object.keys(projectList.projectListObject).length == 0 ||
    Object.keys(JSON.parse(localStorage.getItem("projectListObject")) || {})
      .length == 0
  ) {
...
export function updateLocalStorage() {
  localStorage.setItem(
    "projectListObject",
    JSON.stringify(projectList.projectListObject)
  );
  localStorage.setItem("currentProject", JSON.stringify(currentProject));
  localStorage.setItem(
    "projectCounter",
    JSON.stringify(projectList.projectCounter)
  );
}
