...
function modalContentTask(modalOv) {
  ...
  taskForm.addEventListener("submit", (event) => {
    ...
    displayRender.filterObjArray(
      currentProject,
      `${
        displayRender.filterSelect.options[
          displayRender.filterSelect.selectedIndex
        ].text
      }`
    );
    displayRender.renderProj(currentProject);
  ...
  yesBtn.addEventListener("click", () => {
    ...
    changeCurrentProject(
      projectList.projectListObject[
        Object.keys(projectList.projectListObject)[0]
      ]
    );
    displayRender.renderProj(currentProject);
    displayRender.projectModalContentList.innerText = "";
...
function modalContentEditTask(modalOv, taskKey) {
  ...
  taskForm.addEventListener("submit", function (event) {
    ...
    wrapper.querySelector(".taskPriority").innerText = `${
      taskFormPrSlct.options[taskFormPrSlct.selectedIndex].text
    }`;
    displayRender.PrIconColor(
      `${taskFormPrSlct.options[taskFormPrSlct.selectedIndex].text}`,
      wrapper.querySelector(".fa-exclamation-circle")
    );
    displayRender.PrTextColor(
      `${taskFormPrSlct.options[taskFormPrSlct.selectedIndex].text}`,
      wrapper.querySelector(".taskPriority")
    );
...
function createNewProject(modalOv) {
  ...
  newProjectForm.addEventListener("submit", (event) => {
    ...
    let projectWrapped = (projectList.projectListObject[
      `proj${++projectList.projectCounter}`
    ] = projectItem(
      `${projectFormTitleIn.value}`,
      {},
